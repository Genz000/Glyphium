...
second image and it joins a timeline under the preview: every clip animates
through its own hold, then hands over to the next in the effect's own
pattern. Reframe any image to 1:1, 4:5, 3:2, 16:9 or 9:16 — it's
...
A project is a list of clips on a timeline. Each clip holds on screen for its
own length -- animating the whole time -- then hands over to the next, and the
last hands back to the first so the loop is seamless. Framing is per clip:
scale belongs to the clip, not the project, because two images rarely share
proportions. Selecting a clip moves the playhead to it, so the stage always
shows the clip being edited; the playhead can also be dragged to scrub.
Handovers don't cross-fade: each cell flips at its own threshold, ordered by
the effect driving it -- a wave wipes across, rain falls down its columns,
decode and shimmer dissolve cell by cell -- and cells caught mid-flight
scramble, which is what makes a decode handover read as decode. Decode gets
one complete cycle per hold, so an image decodes itself before it hands over;
it steps aside during the handover, which is already scrambling. The grid is
sized from the first clip, so every other image is placed into the same frame.
